...
def insert_air_temp(dId, lId, zw, mw, at, sst):
    # INSERT INTO `Location` (`id`, `Country`, `City`, `Latitude`, `Longitude`) VALUES (NULL, NULL, NULL, '-0.02', '-109.46');
    try:
        query = "INSERT INTO `Air_Temperature` (`dates_ID`, `location_ID`, `Zonal_Winds`, `Meridional_Winds`, `Air_Temperature`, `Sea_Surface_Temperature`) VALUES (%s, %s, %s, %s, %s, %s)"
        record = (dId, lId, zw, mw, at, sst)
...
with open("../elnino.csv") as csvfile:
    ...
    for row in readCSV:
        ...
        insert_air_temp(d_id, l_id, row[7], row[8], row[10], row[11])
