...
def insert_Temp(date_ID, Loc_id, val, val2, val3, val4, val5):
    ...
    try:
        ...
        query = "INSERT INTO `Corvallis_Snow`(`ID`, `dates_ID`, `location_ID`, `Total_Percipitation_days`, `Multiday_precipitation_total`, `Precipitation`, `Snowfall`, `Snow_depth`) VALUES (NULL, %s, %s, %s, %s, %s, %s, %s)"

        record = (date_ID, Loc_id, val, val2, val3, val4, val5)
        print(record)
...
with open("./Corvallis Weather.csv") as csvfile:
    ...
    for row in readCSV:
        ...
        date = select_Date(data[0], data[1], data[2])

        if date == -2:
            insert_Date(data[0], data[1], data[2])
            date = select_Date(data[0], data[1], data[2])

        loc = select_Location("United States", "Corvallis")

        if(loc == -2):
            insert_Location("United States", "Corvallis")
            loc = select_Location("United States", "Corvallis")

        insert_Temp(date, loc, row[3], row[4], row[5], row[6], row[7])
